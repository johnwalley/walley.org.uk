...
- __Three axes to decide up front__:
  ...
  1. __Tooling__: Vite for apps, Vite library mode for libs
...
- __internally published__ - published to an internal regitry, may, or may not, have a build step
- __externally published__ - published to a public regsitry; always have a build step
...
Run a buld first and export compiled output + declarations.
...
## Axis 3 - Tooling: where Vite, Rollup, and `tsc` first
...
- Libraries: Build JavaScript with Rollup. Emit types with `vite-plugin-dts`.
